...
def main():
    ...
    python convert_hf_to_deepspeed.py ${DATASETS_DIR}/huggingface_transformers/pytorchall_diff/bloom-1b3 \
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(hf_model_name_or_path,torch_dtype=torch.float16)
    ...
    for fn in sorted(os.listdir(checkpoint_dir)):
        ...
        if os.path.isfile(fp) and fn.endswith('model_states.pt') and fn.startswith(LAYER_FILE_PREFIX):
            fn_split = fn.split('-')
            print(MODEL_FILE_PREFIX,fn_split)
